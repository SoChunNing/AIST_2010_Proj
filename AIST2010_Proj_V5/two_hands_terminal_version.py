...
def detect_hand_gesture(mode=0, midinote=81):
    ...
    key_pressed_1 = key_pressed_2 = key_pressed_s = key_pressed_z  = False
    instrument_id = 0
    present_mode_on = 0
    ...
    with mp_hands.Hands(
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                #Gesture replacement of keys
                match gesture_text:
                    case "One":
                        #Replace the 'm' button
                        #Change mode
                        if not key_pressed_1:
                            mode = (mode + 1) % len(mode_id)
                            print("Mode Changed")
                            init = False
                            key_pressed_1 = True
                    case "Two":
                        ...
                        #Change instrument
                        if not key_pressed_2:
                            instrument_id = (instrument_id + 1) % len(instrument)
                            print("Instrument Changed")
                            key_pressed_2 = True
                          
                    case "Cross":
                        #Replace the 'q' button
                        #Exit the program
                        print("Exiting the program...")
                        break
                        # To Oscar: After many test, I found that gesture "cross" will mix-up with other hand gesture. 
                        # Please check if every things works probably in your own laptop, otherwise we couldn't use this anymore! 
                    case _:
                        key_pressed_1 = key_pressed_2 = False
